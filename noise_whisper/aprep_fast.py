...
# db_server url
db_server_url = 'http://db_server:8083/get_simmilar'
...
async def send_message(message: Request):
    ...
    print("[LOG send_message]Message:", data["message"])
    print("[LOG send_message]document_id:", data["document_id"])
    print("[LOG send_message]ai:", data["ai"])
    response = requests.get(db_server_url, json=data)
    text_result = response.json()['text']
    text_for_tts = string_conv(text_result)
    tts_res = tts.apply_tts( tts_model, text_for_tts, sample_rate=SAMPLE_RATE, speaker=SPEAKER)
...
async def speech_to_text(file: UploadFile = File(...)):
    if file is not None:
        ...
        print("[LOG speech_to_text] text:", whisper_result)
